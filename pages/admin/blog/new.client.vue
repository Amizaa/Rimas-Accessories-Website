...
const { createItem, uploadImages, updateItem } = useAdmin()
...
const submitBlog = async () => {
  ...
  formData.append("author", `${user.value.first_name} ${user.value.last_name}`)
  ...
  const response = await createItem('blog-posts',formData)
  ...
  if (response.success) {
    ...
    if (!response2.success) {
      ...
      toast.add({ title: 'ساخت بلاگ', description: 'ساخت بلاگ با موفقیت انجام شد.', color: 'success',onClose: () => navigateTo('/admin/blog')})
    }
...
const checkForm = () =>{
  ...
  if (!sidebarData.value.thumbnail) {
    formError.value.thumbnail = "تصویر بندانگشتی نمی‌تواند خالی باشد."
  }
...
<template>
  <div class="flex justify-between items-center">
    <h1 class="text-4xl font-azarmehrbold my-5">بلاگ جدید</h1>
    <UButton @click="checkForm" size="xl" class="cursor-pointer text-center p-2 bg-indigo-400 hover:bg-indigo-500">
      ساخت بلاگ
    </UButton>
  </div>
  <div class="mt-5 shadow bg-white w-full flex flex-col gap-5 px-3 md:px-16 lg:px-28 md:flex-row">
    <AdminNewBlogSidabar v-model="sidebarData" :post="null" :formError="formError"  />
    <main class="md:w-2/3 lg:w-3/4 w-full py-1 min-h-screen">
      <AdminQuillEditor v-model="editorContent" :post="null" :error="formError.content"/>
    </main>

<script setup>
const props = defineProps({
  post:Object
})

const schema = ref(null)

const state = reactive({
    slug: undefined,
    title: undefined,
    excerpt: undefined,
    thumbnail: undefined,
    category: undefined,
    tags: undefined,
    author: undefined

})

if (props.post) {
  state.slug = props.post.slug
  state.title = props.post.title
  state.excerpt = props.post.excerpt
  // state.thumbnail = props.post.thumbnail
  state.category = props.post.category
  state.tags = props.post.tags
  state.author = props.post.author
}

onMounted(async () => {
  const { default: Joi } = await import('joi')
  schema.value = Joi.object({
  id: Joi.number().integer().min(1).required(),

  slug: Joi.string()
    .pattern(/^[\u0600-\u06FFa-zA-Z0-9\-]+$/)
    .max(100)
    .required(),

  title: Joi.string()
    .max(100)
    .required(),

  excerpt: Joi.string()
    .max(300)
    .required(),

  thumbnail: Joi.string()
    .uri({ allowRelative: true })
    .required(),

  category: Joi.string()
    .max(50)
    .required(),

  tags: Joi.array()
      .items(
          Joi.string()
      )
      .min(1)
      .required()
      .messages({
        'array.base': 'تگ ها باید به صورت لیست وارد شوند',
        'array.min': 'حداقل یک تگ باید وارد شود',
        'any.required': 'وارد کردن تگ ها الزامی است'
      }),

    })

})
    

const category =  [
    {
      label: "تکنولوژی",
      value: "technology"
    },
    {
      label: "سبک زندگی",
      value: "lifestyle"
    },
    {
      label: "سلامت",
      value: "health"
    }
  ]

  
</script>

<template>
    <aside class="w-full md:w-1/3 lg:w-1/4 py-4 md:block shadow-md p-2">
        <div class="sticky top-22">
            <UForm :schema="schema" :state="state" class="space-y-4 space-x-3 my-5" @submit="onSubmit">
                <UFormField size="xl" class="ccol-span-1" label="عنوان" name="title">
                    <UInput v-model="state.title" class="w-full" placeholder="عنوان بلاگ"/>
                </UFormField>
                <UFormField size="xl" class="col-span-1" label="تصویر" name="thumnail">
                      <UFileUpload v-model="state.thumbnail" accept="image/*" class="w-full min-h-48" />
                </UFormField>
                <UFormField size="xl" class=" col-span-2 md:col-span-1" label="تگ ها" name="tags">
                  <UInputTags style="direction:rtl" v-model="state.tags" class="w-full" placeholder="بعد از وارد کردن هر تگ Enter را بزنید"/>
                </UFormField>
                <UFormField size="xl" class=" col-span-2 md:col-span-1" label="دسته بندی" name="category">
                  <USelect style="direction:rtl" v-model="state.category" class="w-full" :items="category"/>
                </UFormField>


                <UButton size="xl" class=" mx-auto cursor-pointer text-center flex justify-center col-span-2 bg-indigo-400 hover:bg-indigo-500" type="submit">
                  ساخت بلاگ
                </UButton>
            </UForm>
            
        </div>
    </aside>
</template>